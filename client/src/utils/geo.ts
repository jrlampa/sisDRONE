/**
 * Simple UTM conversion approximation for WGS84
...
export function degreesToUtm(lat: number, lng: number) {
  ...
  if (lat < 0) {
    utmNorthing += 10000000; // 10,000,000 meter offset for southern hemisphere
  }

  return {
    x: utmEasting.toFixed(2),
    y: utmNorthing.toFixed(2),
    zone: `${zone}${lat >= 0 ? 'N' : 'S'}`
  };
}
