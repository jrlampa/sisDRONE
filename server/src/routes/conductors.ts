 ...
 *   POST   /api/conductors           – criar span (computa computed_length_m via Haversine)
 *   GET    /api/conductors/:id       – buscar por ID
 *   PUT    /api/conductors/:id       – atualizar campos (Phase 32)
...
/** Build the full JOIN SELECT used by GET / and GET /:id */
const CONDUCTOR_SELECT = `
  SELECT
    c.id, c.tenant_id, c.pole_from, c.pole_to,
    c.network_type, c.cable_type, c.voltage_kv, c.length_m, c.computed_length_m, c.notes, c.created_at,
...
router.post('/', rateLimit(30, 60_000), async (req: Request, res: Response) => {
  ...
  try {
    const db = await getDb();
    const poleFrom = await db.get('SELECT id, lat, lng FROM poles WHERE id = ?', [fromId]);
    if (!poleFrom) return res.status(404).json({ error: `Poste de origem (${fromId}) não encontrado` });

    const poleTo = await db.get('SELECT id, lat, lng FROM poles WHERE id = ?', [toId]);
    if (!poleTo) return res.status(404).json({ error: `Poste de destino (${toId}) não encontrado` });

    // Auto-compute span length from pole coordinates (Phase 32)
    let computedLen: number | null = null;
    if (poleFrom.lat != null && poleFrom.lng != null && poleTo.lat != null && poleTo.lng != null) {
      computedLen = parseFloat(haversineMeters(poleFrom.lat, poleFrom.lng, poleTo.lat, poleTo.lng).toFixed(2));
    }

    const result = await db.run(
      `INSERT INTO conductors (tenant_id, pole_from, pole_to, network_type, cable_type, voltage_kv, length_m, computed_length_m, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [tidVal, fromId, toId, netType, sanitizedCableType, vKv, lenM, computedLen, sanitizedNotes]
    );
...
router.get('/:id', rateLimit(60, 60_000), async (req: Request, res: Response) => {
  ...
  try {
    const db = await getDb();
    const conductor = await db.get(`${CONDUCTOR_SELECT} WHERE c.id = ?`, [id]);
...
/**
 * PUT /api/conductors/:id  (Phase 32)
 * Atualiza campos de um condutor. Recalcula computed_length_m se os postes mudarem.
 * Body: { network_type?, cable_type?, voltage_kv?, length_m?, notes? }
 */
router.put('/:id', rateLimit(30, 60_000), async (req: Request, res: Response) => {
  const id = parseInt(req.params.id, 10);
  if (isNaN(id) || id <= 0) return res.status(400).json({ error: 'id inválido' });

  const { network_type, cable_type, voltage_kv, length_m, notes } = req.body;

  if (network_type !== undefined && !isValidNetworkType(network_type)) {
    return res.status(400).json({ error: `network_type inválido. Use: ${VALID_NETWORK_TYPES.join(', ')}` });
  }
  if (voltage_kv !== undefined) {
    const v = parseFloat(String(voltage_kv));
    if (isNaN(v) || v < 0 || v > 500) return res.status(400).json({ error: 'voltage_kv deve ser entre 0 e 500' });
  }
  if (length_m !== undefined) {
    const l = parseFloat(String(length_m));
    if (isNaN(l) || l < 0 || l > 100_000) return res.status(400).json({ error: 'length_m deve ser entre 0 e 100000' });
  }

  const sets: string[] = [];
  const vals: unknown[] = [];

  if (network_type !== undefined) { sets.push('network_type = ?'); vals.push(network_type); }
  if (cable_type !== undefined) { sets.push('cable_type = ?'); vals.push(String(cable_type).slice(0, 100)); }
  if (voltage_kv !== undefined) { sets.push('voltage_kv = ?'); vals.push(parseFloat(String(voltage_kv))); }
  if (length_m !== undefined) { sets.push('length_m = ?'); vals.push(parseFloat(String(length_m))); }
  if (notes !== undefined) { sets.push('notes = ?'); vals.push(String(notes).slice(0, 1000)); }

  if (sets.length === 0) return res.status(400).json({ error: 'Nenhum campo válido para atualizar' });
  ...
  try {
    ...
    vals.push(id);
    await db.run(`UPDATE conductors SET ${sets.join(', ')} WHERE id = ?`, vals);
    const updated = await db.get(`${CONDUCTOR_SELECT} WHERE c.id = ?`, [id]);
    res.json(updated);
  } catch (err) {
